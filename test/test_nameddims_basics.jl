...
@testset "ITensorBase.jl" begin
    @testset "Basic functionality (eltype=$elt)" for elt in TestBasicsUtils.elts
        ...
        @test nametype(na) === String

        # equals (==)/isequal
        a = randn(elt, 3, 4)
        na = NamedTensor(a, ("i", "j"))
        @test na == na
        @test na == align(na, ("j", "i"))
        @test isequal(na, na)
        @test isequal(na, align(na, ("j", "i")))
        @test hash(na) == hash(align(na, ("j", "i")))
        # Regression test that ITensorBase
        # with different names are not equal (as opposed to
        # erroring).
        @test na ≠ NamedTensor(a, ("j", "k"))
        @test !isequal(na, NamedTensor(a, ("j", "k")))
        @test hash(na) ≠ hash(NamedTensor(a, ("j", "k")))
        ...
        a = randn(elt, 2, 2, 2)
        na = NamedTensor(a, ("i", "j", "k"))
        b = randn(elt, 2, 2, 2)
        nb = NamedTensor(b, ("k", "i", "j"))
        copyto!(na, nb)
        @test na == nb
        ...
        @test inds(na[j, i]) == [NamedUnitRange(1:3, "i"), NamedUnitRange(1:4, "j")]
        @test na[j, i] == na
        @test @view(na[j, i]) == na
        @test i[axes(a, 1)] == NamedUnitRange(1:3, "i")
        @test j[axes(a, 2)] == NamedUnitRange(1:4, "j")
        @test axes(na, i) == ai
        @test axes(na, j) == aj
        @test size(na, i) == si
        @test size(na, j) == sj
        ...
        na = NamedTensor(a, ("i", "j"))
        for na′ in (
                na[NamedUnitRange(2:3, "i"), NamedUnitRange(2:3, "j")],
                na["i" => 2:3, "j" => 2:3],
            )
            @test inds(na′) == [NamedUnitRange(1:2, "i"), NamedUnitRange(1:2, "j")]
        ...
        for na′ in
            (
                @view(na[NamedUnitRange(2:3, "i"), NamedUnitRange(2:3, "j")]),
                @view(na["i" => 2:3, "j" => 2:3]),
            )
            @test inds(na′) == [NamedUnitRange(1:2, "i"), NamedUnitRange(1:2, "j")]
        ...
        @test a′ == transpose(a)
        nb = setnames(na, ("k", "j"))
        @test inds(nb) == [NamedUnitRange(1:3, "k"), NamedUnitRange(1:4, "j")]
        @test unnamed(nb) == a
        nb = rename(na, "i" => "k")
        @test inds(nb) == [NamedUnitRange(1:3, "k"), NamedUnitRange(1:4, "j")]
        @test unnamed(nb) == a
        # An index-keyed pair relabels by the index's name.
        nb = rename(na, NamedUnitRange(1:3, "i") => NamedUnitRange(1:3, "k"))
        @test inds(nb) == [NamedUnitRange(1:3, "k"), NamedUnitRange(1:4, "j")]
        @test unnamed(nb) == a
        # The function form maps over the names.
        nb = rename(n -> n == "i" ? "k" : n, na)
        @test inds(nb) == [NamedUnitRange(1:3, "k"), NamedUnitRange(1:4, "j")]
        @test unnamed(nb) == a
        ...
        @test length(na) == 12
        @test Tuple(axes(na)) == (NamedUnitRange(1:3, "i"), NamedUnitRange(1:4, "j"))
        ...
        @test unnamed(na′) isa PermutedDimsArray{elt}
        @test a == permutedims(unnamed(na′), (2, 1))
        # The map form of `align` takes a codomain and a domain tuple. A dense backend
        ...
        @test names(nab) == [name(ci), name(cj)]
        ...
        rng = default_rng()
        @test names(randn(rng, ci, cj)) == [name(ci), name(cj)]
        @test names(randn(rng, (ci, cj))) == [name(ci), name(cj)]
        @test names(randn(rng, (ci,), (cj,))) == [name(ci), name(cj)]
        ...
        na = NamedTensor(randn(elt, 2, 3), (:i, :j))
        ...
        a = NamedTensor(randn(elt, 2, 3), (:i, :j))
        b = NamedTensor(randn(elt, 3, 2), (:j, :i))
        c = a + b
    ...
    @testset "show" begin
        ...
        @test sprint(show, "text/plain", a) ==
            "NamedOneTo(2, \"i\")×NamedOneTo(2, \"j\") " *
            "$NamedTensor{String}:\n" *
            "2×2 Matrix{Int64}:\n 1  2\n 3  4"

        a = NamedTensor([1 2; 3 4], ("i", "j"))
        @test sprint(show, a) ==
            "[1 2; 3 4][NamedOneTo(2, \"i\"), NamedOneTo(2, \"j\")]"
    end
